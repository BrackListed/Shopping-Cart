...
export function Header({searchQuery, setSearchQuery, suggestions, setproductClicked}: HeaderProps) {
  const userCart = useSelector((state: RootState) => state.Cart.cart)
  const cartLength = userCart.length
  const dispatch = useDispatch()
  //only need selectedproduct and productclicked, leave the rest to main
  return (
        ...
        <div className='flex relative justify-center w-full max-w-xl mx-4'>
          ...
          {(suggestions.length > 0 && searchQuery.length > 0) && <div className="absolute top-full left-0 w-full mt-1 bg-white border border-zinc-200 rounded-lg shadow-xl overflow-hidden z-50 flex flex-col">
            {suggestions.map((product) => (
              <div onClick = {() => {dispatch(selectProduct(product.id)), setproductClicked(true)}}className='px-4 py-3 text-zinc-800 text-sm font-medium hover:bg-zinc-100 border-b border-zinc-100 last:border-b-0 cursor-pointer flex items-center gap-3 transition-colors duration-150'>
